...
const Clientes = () => {
  ...
  const fetchClientes = async () => {
    ...
    if (processosError) {
      toast({ title: "Erro ao carregar processos", description: processosError.message, variant: "destructive" });
      return;
    }

    const clientesWithProcessosCount = clientesData?.map(cliente => {
      const processosAtivosCount = processosData?.filter(
        processo =>
          processo.cliente_id === cliente.id &&
          (processo.status === "Em Andamento" || processo.status === "Aguardando")
      ).length || 0;

      return { ...cliente, processos_ativos: processosAtivosCount };
    });

    setClientes(clientesWithProcessosCount ?? []);
  };
        ...
        <AlertDialogContent className="mx-4 max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm text-muted-foreground">
              ...
              <br />• Histórico de Agendados relacionadas
