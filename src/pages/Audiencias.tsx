...
export type Audiencia = {
  ...
  hora: string;
  local: string;
  tipo: "Residencia Temporaria" | "Residencia Permamente" | "Cedula Paraguaia" | "Prorroga Residencia Temporaria" | "Licença de Conduzir";
...
export default function Audiencias() {
  ...
  const handleDelete = async () => {
    if (deletingAudienciaId) {
      ...
      toast.success("Agendamento excluída com sucesso!");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">
            Gerenciamento de Agendados
          </h1>
          <p className="text-sm sm:text-base text-muted-foreground">
            Visualize, adicione e gerencie os Agendados.
          </p>
        </div>
        <Button
          onClick={() => {
