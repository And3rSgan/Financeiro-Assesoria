...
const Agenda = () => {
  ...
  const loadAgendaData = async () => {
    try {
      ...
      console.log('Agendados query result:', { audiencias, audienciasError });
      
      if (audienciasError) {
        console.error('Erro ao buscar Agendados:', audienciasError);
      ...
      // Converter Agendados para AgendaItem
      const audienciasItems: AgendaItem[] = (audiencias || []).map(audiencia => ({
        id: audiencia.id,
        type: 'audiencia',
        title: `Agendamento - ${audiencia.tipo}`,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-muted-foreground">
            Gerencie seus compromissos, Agendados e tarefas.
          </p>
        </div>
      </div>
