...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'audiencia_tipo') THEN
    CREATE TYPE public.audiencia_tipo AS ENUM ('Residencia Temporaria', 'Residencia Permamente', 'Cedula Paraguaia', 'Prorroga Residencia Temporaria', 'Licença de Conduzir');
  END IF;
